...
def GPUtanimoto(query, target, cutoff=0, count=None):
    ...
    tanimoto(drv.In(query), np.int32(len(query)), drv.In(target),
             np.int32(len(target)), np.int32(len(query[0])), drv.Out(dest),
             block=bdim, grid=gdim)
    print "total time time:", time.time() - start_time, "seconds"
    print "Similarity speed",\
        len(query)*len(target)/(time.time() - start_time), "Tanimoto/second"
